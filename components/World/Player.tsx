...
export const Player: React.FC = () => {
  ...
  const lastProcessedJumpSignal = useRef(0);
  ...
  useEffect(() => {
      if (status === GameStatus.PLAYING) {
          ...
          lastProcessedJumpSignal.current = 0;
  ...
  // Watch for global jump signals (Keyboard/Touch/Webcam triggers store update)
  useEffect(() => {
      if (jumpSignal > lastProcessedJumpSignal.current && status === GameStatus.PLAYING) {
          performJump();
          lastProcessedJumpSignal.current = jumpSignal;
      }
  }, [jumpSignal, status]);

  // --- Keyboard & Touch Controls ---
  // Note: We now route these through the Store to keep state synced with Webcam
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (status !== GameStatus.PLAYING) return;
      
      if (e.key === 'ArrowLeft') setTargetLane(currentLane - 1);
      else if (e.key === 'ArrowRight') setTargetLane(currentLane + 1);
      else if (e.key === 'ArrowUp' || e.key === 'w') globalTriggerJump();
  ...
  useEffect(() => {
    ...
    const handleTouchEnd = (e: TouchEvent) => {
        ...
        if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) > 30) {
             if (deltaX > 0) setTargetLane(currentLane + 1);
             else setTargetLane(currentLane - 1);
  ...
  useFrame((state, delta) => {
    ...
    // 1. Horizontal Position (Driven by Store.currentLane)
